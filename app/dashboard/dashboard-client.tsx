...
import { mapearMiembros, type DashboardRaw, type PerfilDashboard } from "@/lib/features/dashboard/carga";
...
export function DashboardClient({ datosIniciales }: { datosIniciales: Promise<DashboardRaw | null> }) {
  const datosInicialesUsados = useRef(false);
  ...
  useEffect(() => {
    ...
    // State updates shared by the server-provided first load and the browser load.
    // Step 1: own profile, banner, admin level, own pending payments.
    const aplicarPaso1 = (
      user: { email?: string | null },
      profile: PerfilDashboard | null,
      elegibles: ElegiblesResult,
      misPagos: number | null
    ) => {
      if (cancelled) return;
      if (profile) {
      ...
      if (misPagos !== null) setMisPagosPendientes(misPagos);
    ...
    // Step 2: figures (computed here, with the browser's local date), recent payments, years, config.
    const aplicarPaso2 = (
      elegibles: ElegiblesResult,
      pagosAnio: PagoRPCRow[],
      pagosResult: Payment[],
      aniosResult: number[],
      ...
      setMiembros(mapearMiembros(elegibles));
      if (config) setGymConfig(config);
    };
      ...
      try {
        ...
        if (!cancelled && profile?.role !== "super_admin") {
          const { data } = await supabase
            ...
            .eq("user_id", user.id)
            .eq("status", "pendiente");
          misPagos = data?.length || 0;
        }
        aplicarPaso1(user, profile, elegibles, misPagos);

        const [pagosAnio, pagosResult, aniosResult, configResult] = await restoPromise;
        aplicarPaso2(elegibles, pagosAnio, pagosResult, aniosResult, configResult.data as GymConfig | null);
      } catch {
    ...
    // First load: use the data the server already fetched (same session, same RLS)
    // when it matches the browser's year; otherwise, or on any problem, load from the browser.
    const cargar = async () => {
      if (!datosInicialesUsados.current) {
        const raw = await datosIniciales; // never rejects
        if (cancelled) return; // StrictMode re-run: the next run applies it
        datosInicialesUsados.current = true;
        if (raw && raw.anio === anioSeleccionado) {
          try {
            const elegibles = filtrarElegibles(raw.elegiblesInput, new Date());
            aplicarPaso1(raw.user, raw.profile, elegibles, raw.misPagosPendientes);
            aplicarPaso2(elegibles, raw.pagosDelAnio, raw.pagosRecientes, raw.anios, raw.gymConfig);
            if (!cancelled) setLoading(false);
            return;
          } catch {
            // fall through to the browser load, which reports errors as before
          }
        }
      }
      await loadData();
    ...
    // datosIniciales is a stable prop for the page's lifetime; only the year triggers reloads
