export function diagnosticoTemplate(
  resultados: Array<{
    paso: string;
  ...
  const filasHtml = resultados
    .map((r) => {
      const icono =
        r.estado === "ok" ? "✅" : r.estado === "warning" ? "⚠️" : "❌";
      const bgColor =
        r.estado === "ok"
          ? "#f0fdf4"
          : r.estado === "warning"
      ...
      return `
      <tr>
        <td style="padding:12px;border-bottom:1px solid #e2e8f0;background:${bgColor};">
          <span style="font-size:14px;">${icono} <strong style="color:#1e293b;">${r.paso}</strong></span>
          <br>
          <span style="font-size:13px;color:#64748b;">${r.detalle}</span>
        </td>
      </tr>`;
    })
    .join("");
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:500px;background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:30px;">
              <h2 style="color:#1e293b;margin:0 0 15px;font-size:20px;">Diagnóstico del Sistema</h2>
              <p style="color:#64748b;font-size:15px;line-height:1.6;margin:0 0 20px;">
                Resultado: <strong style="color:${exitoso ? "#16a34a" : "#dc2626"};">${exitoso ? "✅ Todo OK" : "❌ Problemas hallados"}</strong>
              </p>
