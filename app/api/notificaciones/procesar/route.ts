...
async function ejecutarEstatusSistema(gymConfig: Record<string, unknown>): Promise<number> {
  ...
  await sendSystemStatusEmail(
    ...
    {
      miembrosActivos: miembrosActivos || 0,
      miembrosInactivos: miembrosInactivos || 0,
      pagosAprobadosMes: pagosAprobadosMes || 0,
      pagosPendientesMes: pagosPendientesMes || 0,
      montoCobradoMes: 0,
      montoPendienteMes: 0,
      ultimoPago: ultimoPago?.created_at || null,
      ultimoMiembro: ultimoMiembro?.created_at || null,
      migraciones: migraciones || 0,
    },
    gymConfig.logo_url as string | null,
    (errores || []).map((e) => ({
      error: e.error_detail || "Error desconocido",
      fecha: e.sent_at,
    }))
  );
